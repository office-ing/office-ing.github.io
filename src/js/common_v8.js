...
$(function () {
  ...
  class Util {
    ...
    static init() {
      ...
      Util.init_floating(selector_floating, callback_floating);
      ...
      Util.init_popup(selector_popup, params_popup);
      Util.init_youtube(params_youtube);
    }
     ...
     */
    static init_floating(selector, callback = null) {
      const $btn = $(selector);
      if ($btn[0]) {
        const $window = $(window);
    ...
    static init_popup(selector, params) {
      ...
      // trigger by time
      if (params.hasOwnProperty("waitTime")) {
        setTimeout(function () {
          if (!stopFlg) modal.show();
        }, params.waitTime);
      }
      // close Element
      if (params.hasOwnProperty("closeElement")) {
        const $closeElement = params.closeElement;
        $closeElement.on("click", function (e) {
          modal.hide();
        });
      }
      // showOnVisible
      if (params.hasOwnProperty("showOnVisible") && params.showOnVisible) {
      ...
      // showOnEscape
      if (params.hasOwnProperty("showOnEscape") && params.showOnEscape) {
      ...
      // showOnScroll
      if (params.hasOwnProperty("showOnScroll") && params.showOnScroll) {
        // 閉じるたびに定期タイマーを初期化
    ...
    static init_youtube(params) {
      ...
      if (params.hasOwnProperty("lazyload") && params.lazyload) {
        ...
        window.onYouTubeIframeAPIReady = () => {
          const video_id = $("[data-video-id]").data("video-id");
