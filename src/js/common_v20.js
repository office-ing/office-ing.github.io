...
$(function () {
  ...
  class Util {
    ...
    #init_floating = async (settings) => {
      ...
      // data属性の指定が優先、なければparamsの値を使う
      // data-start属性が空文字列の場合も「指定なし」と見なす
      const startSelector = $target.data("start") === "" ? null : $target.data("start") || start;
      const stopSelector = $target.data("stop") || stop;

      const $start = startSelector ? $(startSelector) : null; // nullの場合がある
      const $stop = $(stopSelector);

      // 開始位置のY座標を計算
      // data-startが指定されていない（startSelectorがnull）場合は常に0（ページの最上部）と見なす
      const getStartTop = () => {
        if (!$start || !$start.length) return 0; // $startがない、または要素がない場合は0
        return $start.offset().top;
      };
      ...
      // 表示条件の決定ロジック
      const shouldShow = (scrollTop) => {
        const startTop = getStartTop(); // $startが指定されていなければ0
        const stopTop = getStopTop();

        // data-startが指定されていない場合 (startSelectorがnull)
        // -> ページの最初から表示し、stopTopで非表示
        if (!startSelector) {
          return scrollTop < stopTop;
        }
        // data-startが指定されている場合
        // -> startTopからstopTopの間で表示
        else {
          return scrollTop > startTop && scrollTop < stopTop;
        }
      };
      ...
      const handleScroll = async () => {
        ...
        const showButton = shouldShow(scrollTop);

        if (showButton && !currentlyVisible) {
          $target.removeClass(hiddenClass).fadeIn();
        } else if (!showButton && currentlyVisible) {
      ...
      // 初期化時の表示状態設定
      // DOM読み込み完了後、最初の描画安定を待ってから実行
      // $(document).ready() 内で呼び出される前提
      const initialScrollTop = $window.scrollTop();
      if (shouldShow(initialScrollTop)) {
        $target.removeClass(hiddenClass).show(); // 即座に表示
      } else {
        $target.addClass(hiddenClass).hide(); // 即座に非表示
      }

      // スクロールイベントを初期表示設定後にバインド
      // これにより、初期ロード時の不要なイベント発火を避ける
