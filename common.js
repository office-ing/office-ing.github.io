$(function () {
  /**
   * 定数定義
   * LPユーティリティクラスの各メソッドに渡す引数や参照する定数を定義します。
   */
  const break_point = 992; // ブレークポイント
  const effective_width = 1100; // 有効幅（モバイルの最大可視範囲）

  /**
   * デバイス判定の設定
   * init_deviceTypeメソッドの引数です。
   */
  const library_devicetype = "UAParser";
  const callback_devicetype = null;

  /**
   * アニメーションの設定
   * init_animationメソッドの引数です。
   */
  const library_animation = "WOW";
  const params_animation = {
    offset: 30,
  };
  const callback_animation = null;

  /**
   * コラプスの設定
   * init_collapseメソッドの引数です。
   */
  const selector_collapse = "#contents";
  const speed_collapse = 5000;
  const once_collapse = false;
  const callback_collapse = null;

  /**
   * アコーディオンの設定
   * init_accordionメソッドの引数です。
   */
  const selector_accordion = "[data-type=accordion]";
  const callback_accordion = null;

  /**
   * スムーススクロールの設定
   * init_smoothscrollメソッドの引数です。
   */
  const speed_smoothscroll = 500;
  const selector_header = "[data-scroll-header]";
  const offset_smoothscroll = 0;
  const callback_smoothscroll = null;

  /**
   * コンテンツ保護の設定
   * init_protectionメソッドの引数です。
   */
  const enable_protection = false;
  const callback_protection = null;

  /**
   * 画像サイズ自動計算の設定
   * init_autofitメソッドの引数です。
   */
  const selector_autofit = "[autofit]";
  const callback_autofit = null;

  /**
   * ページ先頭に戻るボタンの設定
   * init_pagetopメソッドの引数です。
   */
  const selector_pagetop = "[data-type=pagetop]";
  const threshold_pagetop = 100;
  const callback_pagetop = null;

  /**
   * カウントダウンタイマーの設定
   * init_countdownメソッドの引数です。
   */
  const selector_countdown = "[data-type=countdown]";
  // const params_countdown = {
  //   KEY: "cd",
  //   limit: new Date(2023, 5, 30, 23, 59).getTime() - new Date().getTime(),
  //   message: "終了準備中",
  //   interval: 1000,
  //   format: "残りあと<span>dd</span>日<span>hh</span>時間<span>mm</span>分",
  //   useStorage: false,
  // };
  const callback_tick_count = null;
  const callback_stop_count = null;

  /**
   * ローディング画面の設定
   * init_loaderメソッドの引数です。
   */
  const selector_loader = "[data-type=loader]";
  const delay_loader = 0;
  const wait_loader = 0;
  const callback_loader = null;

  /**
   * フローティングバナーの設定
   * init_floatingメソッドの引数です。
   */
  const selector_floating = "[data-type=floating]";
  const callback_click_floating = null;

  /**
   * スライドショーの設定
   * init_sliderメソッドの引数です。
   */
  const selector_slider = "[data-type=slider]";
  // const params_slider = {
  //   fade: true,
  //   autoplay: true,
  //   autoplaySpeed: 5000,
  ...
  const callback_video = null;

  /**
   * フォームの設定
   * init_formメソッドの引数です。
   */
  const selector_form = ".needs-validation";
  ...
  const callback_drawer = null;

  /**
   * パーティクルの設定
   * init_particleメソッドの引数です。
   */
  const selector_particle = "[data-type=particle]";
  const json_path_particle = "./assets/lib/particles/";
  const callback_particle = null;
